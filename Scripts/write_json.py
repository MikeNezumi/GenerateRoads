...
if "." in __name__:  # filed is called by a script outside of Scripts
    from .read_json import get_dict, order_neighbours
    from .abstract_geometry import intersection_point, bezier_intersection, beziers_crossing, offset_polyline, offset_bezier, orient_line
else:
    from read_json import get_dict, order_neighbours
    from abstract_geometry import intersection_point, bezier_intersection, beziers_crossing, offset_polyline, offset_bezier, orient_line
...
replicate_json(file_path):
    takes a one-line json nightmare and creates a readible .txt copy
...
def zoom_graph(data_source, coefficient, copy = False):
    ...
    if file:
        ...
        if copy:
            ...
            print("Extended coordinates in " + data_source + ", made a legible .txt copy")
        else:
            print("Extended coordinates in " + data_source)

    return json_data

#d = zoom_coordinates("../Data/capitalExtended.json", 2)
#data = {
#    "Vertices": {
#        "A" {"Coordinates": [5, 5]},
#        "B" {"Coordinates": [5, 5]},
...
#    }
#}
#zoomed = zoom_coordinates(data, 6)
#for v in zoomed["Vertices"].items():
    #print(v)
#for e in zoomed["Edges"]:
    #print(e)
...
insert_vertices():
    ...
    Input: path to a new .json file
           a properly formatted "Vertices" python dict
...
corners into tiny bezier curves of given maximum curvature (in meters, radius)

Input: a rough rails graph (pythonic dict)
Output: final, smoothened vehicle graph (pythonic dict)
"""
def smoothen_rails(rails_dict, max_curve):
    pass

"""
evaluate_offsets():
Takes a rails graph with "Offsets" on Beziers, turns offset points into new
control points, finds new top control point (using curve tangents)

Input: rails graph with "Offsets" (pythonic dict)
Output:  "Offsets"-free rails graph (pythonic dict)
"""
def evaluate_offsets(edges_dict):
    pass

"""
insert_rails():
Deduces all rails' vertices from graph data and adds them to the json blueprint
Its present inner structure is:
    ...
    7 - (OPTIONAL) Smoothen out all corners
    8 - Recompute beziers to fit inside Offsets
    9 - Write finished "Rails" pyJSON into the source .json file
        * (OPTIONAL) Create legible .txt copy
Nested: strip(label) (str) -> stripped label (str)
Uses: read_json.get_dict()
      read_json.order_neighbours()
      .smoothen_rails()

Input: json file (path str), half_gauge (int) IN METERS, drive-on-right (bool)
...
""" ~~~~~~~~~~ (0 - define nested strip()) ~~~~~~~~~~ """
def insert_rails(file_path, half_gauge, drive_right = True, smooth = True, copy = True):
    # strip():
    ...
    for vertice_key in vertices:
        ...
        for neighbour in neighbours:  # go through all neighbours
            ...
            if "Shape" not in doubled_edge.keys():  # simple line
                ...
                intersection_rails.append(offset_bezier(points, half_gauge, True))  # First append right rail
                intersection_rails.append(offset_bezier(points, half_gauge, False)) # Then append left rail
            ...
            if len(neighbours) == 1:  # this vertice is a dead-end, no intersection point
                ...
                for rail in intersection_rails:
                    ...
                    for end_point in end_points:
                            x_distance = abs(end_point[0] - ref_point[0])
                            y_distance = abs(end_point[1] - ref_point[1])
            ...
            for segment in shape_segments:
                if isinstance(segment[0][0], list):  # Bezier curve, add shifted control points
                    if tuple(original_labels) in complex_edges.keys():  # control points are in the right direction
                        subbeziers = offset_bezier(segment[0], half_gauge, drive_right)
                        ...
                        subbeziers = offset_bezier(segment[0][::-1], half_gauge, drive_right)
                        wrapped_subbeziers = []
                        for subbez in subbeziers:
            ...
            if len(doubled_segments) == 1:  # just 1 segment
                ...
                while doubled_index < len(doubled_segments) - 1:  # list may dynamically expand, this prevents overflow
                    ...
                    if isinstance(segment[0][0], list) and isinstance(next_segment[0][0], list): # segments: bezier ; bezier
                        ...
                        offsets_dict = segment[0][-1] if isinstance(segment[0][-1], dict) else False
    ...
    for rail_index, unchopped_rail in unchopped_shapes.items():
        ...
        if sibling_vertice != " ":  # ignore dead-end vertices, no cutting required there...
            try:
                ...
                #print(rail, end_vertice, sibling_vertice)
                return False
    ...
    """ ~~~~~~~~~~  (OPTIONAL 7 - Smoothen out all corners) ~~~~~~~~~~ """
    # TODO
    """ ~~~~~~~~~~  8 - Recompute beziers to fit inside Offsets  ~~~~~~~~~~ """
    # TODO
    ...
    if copy:
        replicate_json(file_path)
        print("Added Rails to " + file_path + ", made a legible .txt copy")
    else:
...
#print(edges)
#insert_rails("../Data/step2.json", 6, False)
