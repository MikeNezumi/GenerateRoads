...
def angle(A, V, B):
    ...
    a = distance(A, V)
    b = distance(B, V)
    return acos(dot_product / (a * b))  # in radians
...
# SAFE TEST:
#print(includes_point([[100, 20], [161.0, 81.0]], [152.10535095855388, 72.1053509585539]))
...
# SAFE TEST:
#point = intersection_point([[7, 6], [3, 0]], [[7, 2], [8, 1]])
#print(point)

def extended_intersection(variables):
    """
    Takes 1 list of two lines, or prepared variables dict in format:
    ...
    for _ in range(1000):  # Try this thousand times, then break out
        ...
        if variables["x_shift1"] == 0:  # for l_copy1, extend only y coordinate
            ...
            for i in range(2):
                ...
                variables["l_copy" + str(i+1)][1][1] += 1000 * variables["k" + str(i+1)]

# Safe test:
#variables = {
#    'l_copy1': [[100, 20], [150, 70]],
#    'x_shift1': 50, 'y_shift1': 50,
#    'k1': 1.0,
#    'l_copy2': [[140, 90], [70, 120]],
#    'x_shift2': -70, 'y_shift2': 30,
#    'k2': -0.42857142857142855
#}
#print(extended_intersection(variables))
...
# Safe test:
#points = [[97.05825797292724, 110.58834840541455], [87.48629012862573, 58.36254383797443], [177.00735767545203, 39.79001924534846], [98.94662967523483, 12.808987532707135]]
#print(approx_bezier(points, [], offsets=[[97.35647484426991, 95.14676185457145], [100.44185676978866, 13.332805452327557]]))
...
def bezier_intersection(bezier_points, checked_points, step=0.005):  # As it stands, horribly inefficient
    ...
    Uses: ./intersection_point(), ./includes_point()
...
# SAFE TESTS:
#bezier1 = [[[0,5], [6,15], [16,15], [16,5]]]
#bezier2 = [[[4,4], [7,9], [12,5], [20,7]]]
#line1 = [[10,100], [80,0]]
#print(bezier_intersection(bezier1, line1))
#print(bezier_intersection(bezier1, bezier2))
...
# SAFE TEST:
#rail1 = [[[45.5082452542781, 46.51713109568811], [50.41996383311043, 44.86902952956642], [53.85173302533673, 42.68380717695225]],
#[[53.85173302533673, 42.68380717695225], [56.63917502443942, 40.875036360341525], [58.470091793132006, 38.68221716586871]],
#[[58.470091793132006, 38.68221716586871], [59.99604420028194, 36.79266005687997], [60.81139899781816, 34.612616864757726]],
#[[60.81139899781816, 34.612616864757726], [61.57834268749988, 32.41236303248735], [61.59998872194142, 30.006007467041055]]]
#rail2 = [58.30312852468575, 38.87907573968162]
#print(beziers_crossing(rail1, rail2))
...
# Safe test:
#xys = [[50, 0], [100, 40], [50, 80]]
#for line in [shifted_lines(xys, 6, True) + shifted_lines(xys, 6, False)][0]:
#    print(line)
...
#SAFE TEST:
#line = [[1,1], [40,50], [60,20], [80,100]]
#line1, line2 = offset_polyline(line, 2.1)
#print(line1, "-", line2)
...
def offset_bezier(coordinates, half_gauge, right, step = 0.005, split_iterations = 2):
    ...
    Uses: .get_k(), .includes_point(), .intersection_point(), .extended_intersection(),
          .approx_bezier(), .triangle_tangent()
...
# Safe test:
#points = offset_bezier([[50, 80], [100, 140], [40, 160]], 6, True, split_iterations=2)
#points = offset_bezier([[100, 10], [180, 40], [135, 50]][::-1], 3.5, False, split_iterations=2)
#print("\n", points)
...
# SAFE TEST:
# mr = 5.5
# p0, p1, p2 = [0, 0], [25, 10], [25, 0]
# print(check_curvature(mr, p0, p1, p2))

def deduce_p1s(mr, e0, e1, p2):  # TODO
    """
    ...
    if 2 * sin(alpha) > 1:
        ...
        horizon_gama = asin(2 * sin(alpha))  # from sine equation
        horizon_beta = pi - (alpha + horizon_gama)
        horizon2 = sqrt(z_length ** 2 * (5/4 - cos(horizon_beta)))  # from cosine equation
    ...
    for complex_x in imaginary_xs:
        ...
        elif horizon1 <= complex_x.real <= horizon2:
            xs.append(complex_x.real)
    ...
    if horizon2:
        ...
        print(xs)  # TEMP
        pass # TODO I left 'ere

    # possibly, 3rd equation - prepare

def connect_line(point, line, minimal_radius):  # TODO
    """
    Smoothly connects a fixed point to a minimally cut line (minimal cut necessary
    for given minimal radius). Input variables are given in a standardiset value dict:
    Uses: .deduce_polygons()

    Input: point (xy list), line ([xy, xy]), minimal_radius (float)
    Output: {
        ...
        - not compute_minimal: (typically called from .biaxial_connector())
                       deducing 1 polygon determined by input vars, checking its
                       curvature against mr -> returning its dict or False
                       Required: mr, e1, V0, y_point
    """

def connect_bezier(point, line, step, minimal_radius): # TODO
    """
    Smoothly connects a fixed point to a minimally cut line (minimal cut necessary
    for given minimal radius). Input variables are given in a standardiset value dict:
    Uses: .deduce_polygons()
...
def smoothen_lines(line1, line2, minimal_radius):
    """
    ...
    Uses: .get_k(), .extended_intersection()

    Input: line1 ([xy, xy]), line2 ([xy, xy]), minimal_radius (float)
    ...
    arm_length = (cos(alpha / 2) * minimal_radius) / (sin(alpha / 2) ** 2)
    points = []
...
# Safe test:
#line1, line2 = [[100, 20], [150, 70]], [[140, 90], [70, 120]]
#dict = smoothen_lines(line1, line2, 4.5)
#shape = [l for l in dict.values()]


def smoothen_combined(line, bezier, step, minimal_radius):
    """
    ...
    dict (still directed) - {
        ...
        "curve2":[[xy, xy, xy], {"Offsets":[xy, False]}]
        }
    Uses: .distance(), .get_k(), .extended_intersection(), .triangle_tangent(), .approx_bezier()
    Implements: https://algorithmist.wordpress.com/2010/12/01/quad-bezier-curvature/

    Input:  line ([xy, xy]), bezier ([[xy, xy, xy]]), step (float), minimal_radius (float)
    Output: directed modified lines with smoothener (*smoothener dict*) OR False
    """
    ...
    line_k = get_k(*line)
    A = line[0]
    gap = bezier[-1] != line[0] if not offsets[0] else offsets[0] != line[0]


    # Determine the nature of needed smoothener (False, line, Bezier or double Bezier):
    if (not offsets[0] and line_k == get_k(*bezier[:2])) \
    or (offsets[0] and line_k == get_k([bez_points[0], bez_points[1]])):
        ...
        and ((line[1][1] <= bezier[0][1] <= bezier[1][1]) or (line[1][1] >= bezier[0][1] >= bezier[1][1])):
            ...
            if not gap:
                if offsets[0] or offsets[1]:
                    bezier.append({"Offsets":offsets})

                return {"line1": line, "smoothener": False, "curve2": [bezier]}
            # 2: LINE SMOOTHENER:
            elif get_k(line1[0], bezier[1]) == line_k:
    ...
    for i in range(1, len(bez_points) - 1):
        ...
        V = extended_intersection([line, tangent])  # TODO what when yielded False?
        p2 = bez_points[i]
        if V == p2:  # touching, unsmooth bezier
            continue
        ...
        arm_minimal = (cos(alpha / 2) * minimal_radius) / (sin(alpha / 2) ** 2)
...
# SAFE TESTS:
#line = [[200, 50], [200, 100]]  # gap
#bezier = [[[200, 100], [260, 100], [400,30]]]
#sm = smoothen_combined(line, bezier, 0.05, 5.5)


def smoothen_beziers(bezier1, bezier2, step, minimal_radius):
    """
    ...
    Returns conventional smoothener dict: (...anything needed to keep network smooth, except biaxial connectors)
    dict (still directed) - {
        "curve1":[[xy, xy, xy], {"Offsets":[False, xy]}],
        "smoothener": [[xy, xy, xy]] OR [xy, xy] OR False
        "curve2":[[xy, xy, xy], {"Offsets":[xy, False]}]
        }
    Uses: .distance(), .get_k(), .extended_intersection(), .triangle_tangent(), .approx_bezier()
    ...
    Input:  bezier ([[xy, xy, xy]]), bezier ([[xy, xy, xy]]), step (float), minimal_radius (float)
    Output: directed modified lines with smoothener (*smoothener dict*) OR False
    """
    ...
    if get_k(*border_line1) == get_k(border_line1[0], border_line2[1]) == get_k(*border_line2):
        if offsets1[0] or offsets1[1]:
            bezier1.append({"Offsets":[offsets1]})
        if offsets2[0] or offsets2[1]:
            bezier2.append({"Offsets":[offsets2]})
        # 1: NONE (False) SMOOTHENER
        if not gap:
    ...
    for i in range(1, len(bez1_points) - 1):  # i can iterate through for both beziers, cuz step is the same
        ...
        if (suggested_radius >= minimal_radius) \
...
# Safe test:
#bez1 = [[[0,0], [10,20], [20,5]]]
#bez2 = [[[20,5], [25,17], [40,0]]]
#ex = smoothen_beziers(bez1, bez2, 0.07, 58.1)


def biaxial_cutter(shape1, shape2, minimal_radius, step = False):
    """
    ...
    mimnimal radius.
    Uses: .connect_line(), .connect_bezier()
    Includes: cut_gap()

    Input: shape1 ([xy, xy]) OR ([[xy, xy, xy]]),
           shape1 ([xy, xy]) OR ([[xy, xy, xy]]),
           minimal_radius (float)
    Output: {
        "line1" : [xy, xy] OR "curve1" : [[xy, xy, xy], {"Offsets":[False, xy]}]
        "smoothener1" : [xy, xy, xy],
        "smoothener2" : [xy, xy, xy],
        "line2" : [xy, xy] OR "curve2" : [[xy, xy, xy], {"Offsets":[xy, False]}]
    ...
    #
    # Input: gap ([xy, xy]),
    #        shape ([xy, xy, xy] OR [xy, xy],
    #        minimal_radius (float)
    #        step (float OR False)
    # Output: gap ([xy, xy])
    def cut_gap(gap, shape, minimal_radius, step, offsets = False):
        ...
        if step and offsets:
            cut_gap = smoothen_combined(gap, [*shape, {"Offsets":offsets}], step, minimal_radius)["line1"]
        elif step:
            cut_gap = smoothen_combined(gap, [shape], step, minimal_radius)["line1"]
        else:
            cut_gap = smoothen_lines(gap, shape, minimal_radius)["line1"]

        return distance(*gap) - distance(*cut_gap)  # always positive

    # HERE BEGINS biaxial_cutter; protect mutable inputs (keep function pure)
    offsets1 = False
    offsets2 = False
    offsets = [offsets1, offsets2]
    if isinstance(shape1[0][0], list):
        assert step, "Cutting curve, its step must be given ~ abstract_geometry.biaxial_cutter()"
        line1 = False
        offsets1 = [False, False]
        bezier1 = [xy.copy() for xy in shape1[0]]
        if isinstance(bezier1[-1], dict):
            offsets1 = bezier1.pop()["Offsets"]

        e1 = offsets1[1] if offsets1[1] else bezier1[-1]
        bez1_points = approx_bezier(bezier1, [], step, offsets1)
    else:
        bezier1 = False
        line1 = [xy.copy() for xy in shape1]
        e1 = line1[1]

    if isinstance(shape2[0][0], list):
        assert step, "Cutting curve, its step must be given ~ abstract_geometry.biaxial_cutter()"
        line2 = False
        offsets2 = [False, False]
        bezier2 = [xy.copy() for xy in shape2[0]]
        if isinstance(bezier2[-1], dict):
            offsets2 = bezier1.pop()["Offsets"]

        e2 = offsets2[0] if offsets2[0] else bezier2[0]
        bez2_points = approx_bezier(bezier2, [], step, offsets2)
    else:
        bezier2 = False
        line2 = [xy.copy() for xy in shape2]
        e2 = line2[0]

    assert e1 != e2, "No gap to connect with biaxial cutter. ~ abstract_geometry.biaxial_cutter()"
    # establish e1-CP : CP-e2 ratio as the ratio of distances cut by smootheners:
    gap = [e1.copy(), e2.copy()]
    cut_distances = []
    shapes = [
        line1 if line1 else bezier1,
        line2 if line2 else bezier2
    ]
    for shape in shapes:
        i = shapes.index(shape)
        if len(shape) == 2:  # line
            cut_distances.append(cut_gap(gap, shape, minimal_radius, False))
        elif offsets[i] == [False, False]:
            cut_distances.append(cut_gap(gap, shape, minimal_radius, step, False))
        else:
            cut_distances.append(cut_gap(gap, shape, minimal_radius, step, offsets[i]))

    CP_ratio = cut_distances[0] / cut_distances[1]
    ...
    # TODO connect_line() / connect_bezier():

# SAFE TESTS:
#shape1 = [[0, 0], [5, 2]]
#shape2 = [[10, 3], [11, 10]]
#dict = biaxial_cutter(shape1, shape2, 3)


def biaxial_connector(line1, line2, minimal_radius):
    """
    ...
    curvature is indeed small enough (Ie. with higher radius than min_radius).
